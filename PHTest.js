...
// Place Harmonization 
	...
	//	Non Gas Stations:
		...
		// BANKS: 
				...
				if ( !(categories.indexOf("BANK_FINANCIAL") > -1) ) { sidebarMessage.push(alertList[0]); severity = 3; lockOK = 0;}
				else {
...
// Place Area check
			...
			tempCat.indexOf("PARK") > -1  )  ) {
        		...
        		severity = Math.max(3, severity);  lockOK = 0;
			}
		...
		// Address check
			...
			if (!item.attributes.name && !item.attributes.residential) {
				sidebarMessage.push("Place does not have a name."); severity = Math.max(3, severity);  lockOK = 0;
			}
			if (!addr.street || addr.street.isEmpty) {
				sidebarMessage.push("Place does not have a street."); severity = Math.max(3, severity); lockOK = 0;
			}
			if (!item.attributes.houseNumber) {
				sidebarMessage.push("Place does not have a house number."); severity = Math.max(3, severity); lockOK = 0;
				}
			else {
...
// Check for weird HNs        	
			// hnTemp = item.attributes.houseNumber.replace(/[^\d]/g,'');
// 			if (hnTemp != item.attributes.houseNumber || hnTemp > 999999) {
// 				sidebarMessage = ["House number is non-standard.  Correct it and rerun, or manually lock if correct."];
//      			severity = Math.max(3, severity);  lockOK = 0;
// 			}
